...
from bingo.symbolic_regression import ExplicitRegression, ExplicitTrainingData
...
from bingo.symbolic_regression.bayes_fitness.mvn_implicit_bayes_fitness_function import (
    MVNImplicitBayesFitnessFunction as IBFF,
)
from bingo.symbolic_regression.bayes_fitness.implicit_bff_laplace import (
    ImplicitLaplaceBayesFitnessFunction as ILBFF,
)
...
def test_true_model():
    ...
    for i, noise_level in enumerate(noise_levels):
        data = np.load(f"../../../data/circle_data/noisycircledata_{noise_level}.npy")[
            :, :2
        ]
        ...
        optimizer = ScipyOptimizer(
            fitness,
            method="L-BFGS-B",
            param_init_bounds=[-1.0, 1.0],
            options={"maxiter": 500},
        )
        MLEclo = LocalOptFitnessFunction(fitness, optimizer)

        ibff = IBFF(
            PARTICLES, MCMC_STEPS, ESS_THRESHOLD, implicit_data, MLEclo, ensemble=1
        )
        ilbff = ILBFF(PARTICLES, MCMC_STEPS, ESS_THRESHOLD, implicit_data, MLEclo)
        ...
        model = PytorchAGraph(equation="(X_0 - 0.0)^2 + (X_1 - 0.0)^2 - (0.0)^2")
        stime = time.time()
