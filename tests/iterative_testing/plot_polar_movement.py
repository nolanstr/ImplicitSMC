...
if __name__ == "__main__":
    ...
    t = np.arctan2(data[:,1], data[:,0])
    plt.plot(t, r, color=plt.cm.tab20(0), label=f"iter 0")
    for i in range(10):
        implicit_data = ImplicitTrainingData(data, np.empty_like(data))
        fitness = MLERegression(implicit_data)

        x_pos, x_neg = fitness.estimate_dx(circle)
        ...
        plt.plot(t, r, color=plt.cm.tab20(i+1), label=f"iter {i}")
    plt.show() 
