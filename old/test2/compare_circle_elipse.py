...
class ImplicitLikelihood(BaseLogLike):
    ...
    def estimate_likelihood(self, inputs):
        ...
        for i, val in enumerate(vals):
            ...
            L2 = np.sum(f**2/np.linalg.norm(df_dx, axis=1, ord=2))
            L2s[i] = L2
...
if __name__ == "__main__":
    ...
    elipse = AGraph(equation=
                "(((X_0 - 1.0) ** 2) / 1.0) + (((X_1 - 1.0) ** 2) / 1.0) - 1")
    lower = 10
    models = [circle, elipse]

    for model in models:
        n = model.get_number_local_optimization_params()
        MLEclo(model)
        if n > 2:
            model.set_local_optimization_params([0.001,1,0.001,1])
        priors = [norm(loc=mu, scale=0.1*abs(mu)) for mu in \
                        model.get_local_optimization_params()] + [uniform(0,1)]
        import pdb;pdb.set_trace()
        print(str(model))
        run_SMC(model, implicit_data, priors)
